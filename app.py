...
if "quiz_state" not in st.session_state:
    st.session_state["quiz_state"] = "setup"  # setup or review
...
if st.session_state["quiz_state"] == "setup":
    ...
    st.session_state["num_questions"] = st.selectbox("Number of quiz questions", options=range(1, 6), index=4)
    ...
    if st.button("🎉 Generate Quiz!"):
        with st.spinner("Generating quiz questions..."):
            try:
                questions = llm_operator.generate_questions_JSON(topic, difficulty, st.session_state["num_questions"])
...
# Quiz review phase
if st.session_state["quiz_state"] == "review":
    ...
    if st.button("✔️ Check All Answers"):
        ...
        st.write(f"🎉 You got {correct_answers} out of {st.session_state['num_questions']} correct in {total_time.seconds} seconds.")
        if correct_answers == st.session_state["num_questions"]:
            st.balloons()
        ...
        # # Add a button to go back to quiz setup
        # if st.button("🔄 Go Back to Quiz Setup"):
